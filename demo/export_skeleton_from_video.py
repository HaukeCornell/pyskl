...
# Forked from https://github.com/kennymckormick/pyskl/blob/main/demo/demo_skeleton.py
import argparse
import cv2
import mmcv
import numpy as np
...
def main():
    ...
    keypoints_output_path = args.out_filename.rsplit('.', 1)[0] + '_keypoints.json'
    keypoints_data = []

    for frame_idx, poses in enumerate(pose_results):
        frame_data = {'frame': frame_idx, 'poses': []}
        for pose in poses:
            keypoints = pose['keypoints'].tolist()  # Convert numpy array to list for JSON serialization
            frame_data['poses'].append(keypoints)
